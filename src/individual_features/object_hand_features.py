...
from src.utils import parse_config, contain_substr
import pandas as pd
import numpy as np
...
def gen_objhand_feature(args, run, tag):
    try:
        ...
        with open(f'output/objhand_complete_{tag}.txt', 'a') as f:
            f.write(run + '\n')
        return track_csv, skel_csv, output_csv
    except Exception as e:
        with open(f'output/objhand_error_{tag}.txt', 'a') as f:
            f.write(run + '\n')
            f.write(repr(e) + '\n')
...
if __name__ == '__main__':
    ...
    if os.path.exists(f'output/objhand_error_{args.feature_tag}.txt'):
        os.remove(f'output/objhand_error_{args.feature_tag}.txt')
