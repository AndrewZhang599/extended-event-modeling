...
def gen_optical_features(args, run, tag):
    try:
        ...
        with open(f'output/optical_complete_{tag}.txt', 'a') as f:
            f.write(run + '\n')
        return input_video_path, output_csv_path
    except Exception as e:
        with open(f'output/optical_error_{tag}.txt', 'a') as f:
            f.write(run + '\n')
            f.write(repr(e) + '\n')
...
if __name__ == '__main__':
    ...
    if os.path.exists(f'output/optical_complete_{args.feature_tag}.txt'):
        os.remove(f'output/optical_complete_{args.feature_tag}.txt')
    if os.path.exists(f'output/optical_error_{args.feature_tag}.txt'):
